...
Send::Send(double delay) : Action(SEND)
{
    // set the delay
    if (delay < 0.0) {
        throw cRuntimeError("[Send::Send(double)] Error, delay can't be a negative number");
    }
    this->delay = delay;
}
...
void Send::execute(cMessage* packet) const
{
    ...
    int fieldIndex = descriptor->findField("sended");
    
    // can't found the field 'sended'
    if (fieldIndex == -1) {
        throw cRuntimeError("[void Send::execute(cMessage*)] Error, field 'sended' not found in the current packet");
    }
	// set the field 'sended', in this way the packet will be sent by the local-filter
	descriptor->setFieldValueAsString(packet, fieldIndex, 0, "1");
}
