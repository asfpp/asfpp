...
void Move::execute()
{
    ...
    if (mobilityModule==nullptr) {
        ...
        if (targetNode->getClassName() == "GlobalFilter") {
            throw cRuntimeError("[Move::execute()] Error, global-filter can't perform the action 'move'");
        }
        else {
            throw cRuntimeError("[Move::execute()] Error, mobility module not found, please check the node structure in the .ned file");
        }
    }
