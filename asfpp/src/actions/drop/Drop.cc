...
Drop::Drop(double threshold) : Action(DROP)
{
    ...
    if ((threshold < 0.0) || (threshold > 1.0)) {
        throw cRuntimeError("[Drop::Drop(double)] Error, threshold must be in the range [0,1]");
    }
    // set the threshold
...
void Drop::setThreshold(double threshold)
{
    // raise error if the threshold is not in the range [0.0, 1.0]
    if ((threshold < 0.0) || (threshold > 1.0)) {
        throw cRuntimeError("[void Drop::setThreshold(double)] Error, threshold must be in the range [0,1]");  
    }
