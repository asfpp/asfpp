...
void WirelessChannel::initialize(int stage)
{
	...
	// <A.P.> Retrieve this parameter in order to determine if attack evaluation is actually performed. In such a case, cell construction has to be performed in a different way. This allows for taking into account destruction and/or misplacement of sensor nodes.
	bool attacksEvaluation = getParentModule() -> par("attacksEvaluation");
	
	// <A.P.> The condition has been extended properly, taking into account attack evaluation.
	if (onlyStaticNodes && !attacksEvaluation  ) {
	...
	topo->extractByNedTypeName(cStringTokenizer("castalia.node.Node").asVector());
...
void WirelessChannel::handleMessage(cMessage * msg)
{
	...
	case WC_NODE_MOVEMENT:{
	     ...
	     *****************************************************/
			...
			// <M.T.> Retrieve this parameter in order to determine if attack evaluation is actually performed.
			bool attacksEvaluation = getParentModule() -> par("attacksEvaluation");

			// <M.T.>  The condition has been extended properly, taking into account attack evaluation.
			if (onlyStaticNodes && !attacksEvaluation)
