//
// Generated file, do not edit! Created by nedtool 5.4 from physicalProcess/PhysicalProcessMessage.msg.
//
...
PhysicalProcessMessage::PhysicalProcessMessage(const char *name, short kind) : ::omnetpp::cMessage(name,kind)
{
    this->value = 0;
    this->xCoor = 0;
    this->yCoor = 0;
...
const char *PhysicalProcessMessageDescriptor::getFieldDynamicTypeString(void *object, int field, int i) const
{
    omnetpp::cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount())
