...
void LocalFilter::handleMessage(cMessage* msg)
{
	...
	if (msg->isSelfMessage()) {
		...
		switch(msgKind) {
            ...
            default: {
                throw cRuntimeError("[void LocalFilter::handleMessage(cMessage*)] Error, kind of self-message not recognized");
            }
...
void LocalFilter::finishSpecific()
{
    ...
    if (resource == nullptr) {
        throw cRuntimeError("[void LocalFilter::finishSpecific()] Error, ResourceManager not found");
    }
...
RadioStats_type LocalFilter::getRadioStats()
{
    ...
    if (communication == nullptr) {
        throw cRuntimeError("[RadioStats_type LocalFilter::getRadioStats()] Error, node's Communication module not found");
    }
    
    // retrieve the radio module
    Radio* radio = check_and_cast<Radio*>(communication->getSubmodule("Radio"));
    if (radio == nullptr) {
        throw cRuntimeError("[RadioStats_type LocalFilter::getRadioStats()] Error, node's Radio module not found");
    }
