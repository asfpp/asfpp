...
void VirtualApplication::initialize()
{
	...
	trace()<<"-> setTimerDrift("<<cpuClockDrift<<")";
	...
	startup_delay += dblrand() * (double)parent->par("startupRandomization");
	 ...
	 */
	latencyMax = hasPar("latencyHistogramMax") ? (double)par("latencyHistogramMax") : 0;
