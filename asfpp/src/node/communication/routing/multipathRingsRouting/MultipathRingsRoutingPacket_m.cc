...
MultipathRingsRoutingPacket::MultipathRingsRoutingPacket(const char *name, short kind) : ::RoutingPacket(name,kind)
{
...
const char *MultipathRingsRoutingPacketDescriptor::getFieldDynamicTypeString(void *object, int field, int i) const
{
    omnetpp::cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount())
