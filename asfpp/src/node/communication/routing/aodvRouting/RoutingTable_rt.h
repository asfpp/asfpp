...
typedef struct _ROW {
        ...
        bool isDestinationValid;	//seq number valido
        RoutingFlag state;
        int hopCount;
...
public:
    ...
    /* This function return TRUE if exist at least one valid 
       route in the routing table. Otherwise return FALSE.
	A.P.
    */
    bool existSomeValidRoute();

    /* Return the RT entry about a destination A.P.*/
    const Entry* getEntry(const char* destination);
