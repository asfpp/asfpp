...
TMacPacket::TMacPacket(const char *name, short kind) : ::MacPacket(name,kind)
{
    this->type = 0;
    this->nav = 0;
    this->sync = 0;
...
const char *TMacPacketDescriptor::getFieldDynamicTypeString(void *object, int field, int i) const
{
    omnetpp::cClassDescriptor *basedesc = getBaseClassDescriptor();
    if (basedesc) {
        if (field < basedesc->getFieldCount())
