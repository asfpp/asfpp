...
void TMAC::startup()
{
	...
	resyncTime = (double)par("resyncTime");
...
void TMAC::fromNetworkLayer(cPacket * netPkt, int destination)
{
	...
	/* The destination must be set after the encapsulatedPacket
		cause it set the destination as BROADCAST.	
	*/
...
void TMAC::updateScheduleTable(simtime_t wakeup, int ID, int SN)
{
	...
	}else{ // you should send your SYNC with your schedule A.P.

		// create the packet
		TMacPacket* syncPkt = new TMacPacket("TMAC SYNC packet", MAC_LAYER_PACKET);
		syncPkt->setType(SYNC_TMAC_PACKET);
		syncPkt->setDestination(BROADCAST_MAC_ADDRESS);
		syncPkt->setSyncId(scheduleTable[0].ID);
		syncPkt->setSequenceNumber(scheduleTable[0].SN);
		syncPkt->setSync(currentFrameStart + frameTime - getClock() - TX_TIME(syncPacketSize));
		syncPkt->setByteLength(syncPacketSize);

		//send it
		toRadioLayer(syncPkt);
