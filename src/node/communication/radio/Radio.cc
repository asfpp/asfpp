...
void Radio::initialize()
{
	...
	/* Variables used to compute the extra energy consumption and 
	   the state time of the Radio. <A.P.>*/
	bytesRXed = 0;
	timeRX = 0;
	timeTX = 0;
	timeSLEEP = 0;
	rxPowerConsumption = RXmode->power;
	rxDatarate= RXmode->datarate;

	declareOutput("Duty cycle");
		...
		case WC_SIGNAL_START:{
			...
			if ((RXmode->modulation == newSignal.modulation) && (newSignal.power_dBm >= RXmode->sensitivity))
				...
				if (newSignal.power_dBm < RXmode->sensitivity) {
					...
					trace() << "Packet from node "<<newSignal.ID<<" has power "<<newSignal.power_dBm;
					trace() << "Radio Sensitivity "<<RXmode->sensitivity;
				}
				else {
		...
		case WC_SIGNAL_END:{
			...
			if (endingSignal->bitErrors != ALL_ERRORS) {
				if (endingSignal->bitErrors <= maxErrorsAllowed(endingSignal->encoding)) {
					...
					trace() << "\t-> Send pkt to MacModule";

					// retrieve the packet's size (A.P.)
					bytesRXed += macPkt->getByteLength();
...
void Radio::completeStateTransition()
{
    ...
    simtime_t  amountOfTimeSpent = simTime() - startOfState;

	switch (state) {

		case TX:{

			timeTX += amountOfTimeSpent; // <A.P.>
		...
		case SLEEP:{
			...
			timeSLEEP += amountOfTimeSpent; // <A.P.>
...
void Radio::finishSpecific()
{
    ...
    switch (state) {
        ...
        case TX:{
            timeTX += amountOfTimeSpent;
            break;
        }
        case RX: {
            timeRX += amountOfTimeSpent;
            break;
        }
        case SLEEP:
        {
            timeSLEEP += amountOfTimeSpent;
            break;
        }
    }

	// end Radio state time
	...
	/*The code used to compute the output duty cycle is made by 
		Alessandro Camillo. You can find it in a post on Castlia
		Google group. */

	simtime_t totalTime = timeTX + timeRX + timeSLEEP;
    collectOutput("Duty cycle", "RX Time", timeRX.dbl());
    collectOutput("Duty cycle", "TX Time", timeTX.dbl());
    collectOutput("Duty cycle", "SLEEP Time", timeSLEEP.dbl());
    if (totalTime > 0)
        collectOutput("Duty cycle", "Duty cycle", 100 * (timeTX + timeRX) / totalTime);
}
...
double Radio::popAndSendToWirelessChannel()
{
	...
	/* Set the field "filtered" to false in the current packet and all possible encapsulated packets <A.P.> */
	setFilteredRecursively(macPkt, false);
	...
	stats.transmissions++;
	trace() << "-> TXed "<<stats.transmissions << " Src "<< macPkt->getSource() << " Dst "<<macPkt->getDestination();
	trace() << "Sending Packet, Transmission will last " << txTime << " secs " << "end->getByteLength(): " << end->getByteLength();
	return txTime;
}
