...
class AodvRouting: public VirtualRouting {
 ...
 private:
	...
	// store the number of rreq retry
	map<string, int> rreqRetryCount;

	// store the "expiration" time for the RT entry
	...
	// contains informations about rreq which are arrived to the node
	priority_queue <RreqExpireTimerSet,
					vector<RreqExpireTimerSet>,
					RreqExpireLifetimeCompare> rreqBroadcast;

	// store the rreq packet for which the node is waiting the rrep
	priority_queue <RreqExpireTimerSet,
					vector<RreqExpireTimerSet>,
					RreqExpireLifetimeCompare> rreqRequest;

	// buffers for rreq and rerr
	...
	// hello message entry lifetime update count
	...
	// lifetimes for expiration of rrep-ack and for the rreq in the blacklist
	priority_queue <RoutingTableEntryExpireTimerSet,
					vector<RoutingTableEntryExpireTimerSet>,
	...
	/*print the information stored in a rt entry identified by the destination.
	  It is used just for  debug. A.P. */
	void printRTEntry(string dest, string what);
