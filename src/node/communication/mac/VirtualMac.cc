...
void VirtualMac::encapsulatePacket(cPacket * pkt, cPacket * netPkt)
{
	...
	// <R.D.>
	// macPkt->setByteLength(macFrameOverhead); <-- original line
	macPkt->setByteLength(pkt->getByteLength() + macFrameOverhead);
	// End of <R.D.> section
	...
	/* Set the packet as filtered by the current node <A.P.> */
	macPkt->setFiltered( ((RoutingPacket*)(netPkt))->getFiltered() );

	// Inherit the flag compromised from the application packet A.P.
	macPkt->setCompromised( ((RoutingPacket*)(netPkt))->getCompromised() );
