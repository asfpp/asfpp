...
void SensorManager::handleMessage(cMessage * msg)
{
	...
	switch (msgKind) {
		...
		case PHYSICAL_PROCESS_SAMPLING:
		{
			...
			/* M.T. DEBUG */
			//trace() << "the value before " << theValue << " sensor index " << sensorIndex;
			...
			/* M.T. DEBUG */
			//trace() << "the value after 1 " << theValue;
...
/* <A.P.> */
double SensorManager::getSensorDeviceSensitivity(int index)
{	
	Enter_Method("getSensorDeviceSensitivity()");
	if ((totalSensors <= index) || (index < 0)) {
		trace() << "WARNING: Sensor index out of bound ( direct method call : getSensorDeviceSensitivity() )";
		return -1.0f;
	} else
		return sensorSensitivity[index];
}

/* <A.P.> */
double SensorManager::getSensorDeviceSaturation(int index)
{	
	Enter_Method("getSensorDeviceSaturation()");
	if ((totalSensors <= index) || (index < 0)) {
		trace() << "WARNING: Sensor index out of bound ( direct method call : getSensorDeviceSaturation() )";
		return -1.0f;
	} else
