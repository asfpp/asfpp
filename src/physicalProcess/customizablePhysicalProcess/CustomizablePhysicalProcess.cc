			...
			const double &y_coo, const simtime_t & stime)
{
	...
	if (stime - time >= SIMTIME_STEP) {
		...
		/* TODO Avoid unnecessary updates of source states (flag vector) */
		...
		for (i = 0; i < numSources; i++) {
			if (source_index[i] >= 0) {
				...
				if (linearUpdate) {
				     ...
				     curr_source_state[i].x += linear_coeff *
				      (sources_snapshots[i][source_index[i] + 1].x -
				      sources_snapshots[i][source_index[i]].x);

				      curr_source_state[i].y += linear_coeff *
				      (sources_snapshots[i][source_index[i] + 1].y -
				      sources_snapshots[i][source_index[i]].y);

				     curr_source_state[i].value += linear_coeff *
	...
	for (i = 0; i < numSources; i++) {
		if (source_index[i] >= 0) {
			...
			/* M.T. DEBUG */
			//trace() << "i " << i << " x_coo " << x_coo << " curr_state.x " << curr_source_state[i].x << " y " << y_coo << " curr_state.y " << curr_source_state[i].y << " value " << curr_source_state[i].value;
		}
	}
