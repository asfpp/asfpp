		...
		*/
		if(detail->get_name() == "action") {
			...
			if(action_name == "Fakeread"){
				...
				int sensorID = (atoi)(tokens[0].c_str());
				string noise = tokens[1].c_str();
				
				vector<double> noiseParams;
				
				// fakeread is an overloaded method
				switch( (int)tokens.size() ){
				
					case Fakeread::fakereadArgc4: 
						noiseParams.push_back( (atof)(tokens[2].c_str()) ); // alpha
						noiseParams.push_back( (atof)(tokens[3].c_str()) ); // beta
						break;
						
					case Fakeread::fakereadArgc5:
						noiseParams.push_back( (atof)(tokens[2].c_str()) ); // alpha
						noiseParams.push_back( (atof)(tokens[3].c_str()) ); // beta
						noiseParams.push_back( (atof)(tokens[4].c_str()) ); // gamma
						break;
						
					case Fakeread::fakereadArgc6:
						noiseParams.push_back( (atof)(tokens[2].c_str()) ); // alpha
						noiseParams.push_back( (atof)(tokens[3].c_str()) ); // gamma
						noiseParams.push_back( (atof)(tokens[4].c_str()) ); // deltaH
						noiseParams.push_back( (atof)(tokens[4].c_str()) ); // deltaL
						break;
						
					case Fakeread::fakereadArgc7:
						noiseParams.push_back( (atof)(tokens[2].c_str()) ); // alpha
						noiseParams.push_back( (atof)(tokens[3].c_str()) ); // beta
						noiseParams.push_back( (atof)(tokens[4].c_str()) ); // tau
						noiseParams.push_back( (atof)(tokens[5].c_str()) ); // phi
						noiseParams.push_back( (atof)(tokens[6].c_str()) ); // gamma
						break;
				};

				Fakeread* fakeread = new Fakeread(sensorID, noise, noiseParams);
				
				attack -> addAction(fakeread);

				//debug<<"\t-> Add Fakeread Action"<<endl;				
